...
class PageItemsRemoteDataSource {
  ...
  Future<List<PageItemModel>> listPageItems({
    ...
    if (!apiResponse.success || apiResponse.data == null) {
      throw Exception(apiResponse.error?.message ?? 'Failed to load page items');
    }
  ...
  Future<PageItemModel> createPageItem({
    ...
    if (!apiResponse.success || apiResponse.data == null) {
      throw Exception(apiResponse.error?.message ?? 'Failed to add page item');
    }
  ...
  Future<PageItemModel> updatePageItem({
    ...
    if (!apiResponse.success || apiResponse.data == null) {
      throw Exception(apiResponse.error?.message ?? 'Failed to update page item');
    }
  ...
  Future<void> deletePageItem({
    ...
    if (response.statusCode != 204) {
      throw Exception('Failed to delete page item');
    }
  }
}
