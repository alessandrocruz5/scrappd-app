...
class ItemsRemoteDataSource {
  ...
  Future<ItemModel> createItem({
    ...
    return runWithFriendlyErrors(() async {
      ...
      if (EnvironmentConfig.verboseLogging) {
        ...
        debugPrint(
          '🧾 Item upload completed in ${elapsed.inSeconds}s '
      ...
      if (!apiResponse.success || apiResponse.data == null) {
        throw FriendlyException(apiResponse.error?.message ?? 'Upload failed.');
      }
  ...
  Future<void> deleteItem(String itemId) async {
    return runWithFriendlyErrors(() async {
      final response = await _dio.delete(ApiConstants.itemById(itemId));
      ...
      if (!apiResponse.success) {
        throw FriendlyException(
          apiResponse.error?.message ?? 'Failed to delete item.',
        );
      }
  ...
  Future<void> cancelItem(String itemId) async {
    return runWithFriendlyErrors(() async {
      final response = await _dio.post(ApiConstants.itemCancel(itemId));
      ...
      if (!apiResponse.success) {
        throw FriendlyException(
          apiResponse.error?.message ?? 'Failed to cancel processing.',
        );
      }
    }, defaultMessage: 'Failed to cancel processing.');
  }
}
