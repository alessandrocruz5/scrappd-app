...
docker build --platform linux/amd64 -t scrappd-api:vN .
docker tag scrappd-api:vN asia-southeast1-docker.pkg.dev/scrappd-prod/scrappd-repo/scrappd-api:vN
docker push asia-southeast1-docker.pkg.dev/scrappd-prod/scrappd-repo/scrappd-api:vN

gcloud run deploy scrappd-api \
  --image=asia-southeast1-docker.pkg.dev/scrappd-prod/scrappd-repo/scrappd-api:vN \
  --region=asia-southeast1 \
  --platform=managed \
  --memory=512Mi \
  --cpu=1 \
  ...
  --set-env-vars="ML_SERVICE_URL=${ML_SERVICE_URL},ENVIRONMENT=production"